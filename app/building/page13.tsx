...
export default function BuildingWithSmoothAnimation() {
  ...
  const playFramesToKeyFrame = (targetFrame, action) => {
    ...
    const animate = () => {
      setFrame((prevFrame) => {
        ...
        console.log("inside set frame")
        console.log("target frame", targetFrame)

        if (action == "prev") {
        console.log("inside prev")
        console.log("target frame prev", targetFrame)

          direction = -1;
        }
        if (action == "next") {
        console.log("inside next")
        console.log("target frame next", targetFrame)
  ...
  const handleNext = () => {
    ...
    playFramesToKeyFrame(nextFrame, "next");
  };
  ...
  const handlePrev = () => {
    ...
    playFramesToKeyFrame(prevFrame, "prev");
  };
